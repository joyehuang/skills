---
name: interview-kb
description: 面经统一管理：接收并归档面试记录（本地 md/txt、飞书文档/妙记），按公司/日期分类存入 ~/research/面经/，维护 README 台账，可生成脱敏公开版发布到 joyehuang.dev。当用户发来面经/面试记录/沟通总结/岗位信息，要求整理面经，**或要求搜索/调研某家公司（尤其是要面试的公司背景调查）**时使用——公司调研结果沉淀到 公司调研/ 目录并更新台账。不要用于：非面试相关的文档管理、飞书文档内容编辑（走 lark-doc）。
---

# Interview KB（面经管理）

## 是什么

统一管理 Joye 的面试资产：面经（完整/浓缩）、沟通记录、岗位信息、会议妙记。私有版存本地，公开版脱敏后发 joyehuang.dev。

## 目录结构

```
~/research/面经/
├── README.md           ← 总台账（索引所有记录，状态图例）
├── 沟通记录/           ← 与 HR/猎头/创始人的沟通总结
│   └── YYYY-MM-DD-公司-对象-主题.md
├── 已面试/             ← 面经记录
│   └── YYYY-MM-DD-公司-岗位方向.md
├── 岗位信息/           ← 岗位/JD
│   └── YYYY-MM-DD-公司-岗位.md
├── 公司调研/           ← 搜公司沉淀（一家一个文件，不按日期命名）
│   └── {公司名}.md
└── 待整理/             ← 收到的原始材料暂存（未分类）
```

命名规范：`YYYY-MM-DD-公司-类型.md`，类型 = 面经/沟通/岗位/妙记。

## 收件流程（用户发来任何面试相关材料时）

1. **识别来源类型**：
   - 本地 md/txt → 直接读取
   - 飞书文档 URL（/docx/ /wiki/）→ `lark-cli docs +fetch --doc <url> --doc-format markdown --as user`（personal profile，用户需授权 docs/drive）
   - 飞书妙记 URL（/minutes/）→ `lark-cli minutes +detail`（personal profile，需 minutes scope）
   - 纯文本/截图 → 转文字后存待整理
2. **归档**：按内容归类到对应目录，命名 `YYYY-MM-DD-公司-类型.md`，文件头带元信息（来源 URL、时间、参与人、状态）
3. **更新 README 台账**：按公司分节，追加一行进展时间线（日期 + 事件 + 链接）
4. **确认**：回复用户归档位置 + 台账摘要

## 台账组织（README 用）

按公司分节，每节下用时间线列出与该公司的接触记录（沟通/面经/岗位调研/进展）。
不设全局状态列——所有记录本身即表示感兴趣，进展用时间线体现。

## 小红书面经发布格式（2026-08-25 用户确认的统一格式）

用户要求发小红书的面试记录用**纯问题版**格式：
1. 只提取问题，去掉回答
2. 分组：**面试官提问** + **候选人反问**（我提问面试官）
3. 每组内按顺序编号
4. 文件名：`{公司}{轮次}-纯问题版.md`，存 `待整理/`
5. 小标题直接写公司+轮次（如 `# 石墨文档出海 AI 产品 · 三面问题集`）

案例：`待整理/石墨三面-纯问题版.md`

## 公司调研沉淀（2026-08-27 建立）

用户会经常让 agent 搜索要面试的公司或想了解的公司（原话：「以后我会经常让你搜索我要面试的公司…这里感觉也需要做一个沉淀」）。流程：

1. **搜索前先查 `~/research/面经/公司调研/{公司名}.md` 是否已有档案** —— 有则基于存量增量更新，不重复全量调研。
2. **搜索后写入/更新** `{公司名}.md`（不按日期命名，一家一份持续更新），结构：
   - 头部元信息：调研日期 ｜ 目标岗位/薪资 ｜ 状态（待面试/已面试/放弃…）
   - 公开信息（带来源链接）、判断（规模/赛道/薪资对比/风险点）、面试待核实清单（checkbox）、进展时间线（每次接触追加一行）
3. **更新 README 台账**：新公司在台账加一节；已有公司追加时间线。
4. 回复用户时给结论 + 告知归档位置。

案例：`公司调研/炅炅有神.md`

## 脱敏公开版（发布 joyehuang.dev）

用户要求公开面经时：
1. 复制私有版 → 去除：个人信息（姓名/学校/坐标/可到岗时间）、面试官名、公司名（或改为"某公司"）、具体业务数据（ARR/内部指标）、未公开产品细节
2. 保留：问题清单、回答思路、技术复盘、踩坑点
3. 存 `~/research/面经/public/`（脱敏后），命名加 `-public` 后缀
4. 发布到 joyehuang.dev（发布方式见下）

## 发布到 joyehuang.dev

joyehuang.dev 部署在 Cloudflare Pages（CF 账号 Huangdeshiou@gmail.com，Free plan）。
- 静态内容：wrangler/直接上传，或走 R2 公网链接
- 具体发布命令待 joyehuang.dev 站点搭建后补充（TODO）

## 验证清单

- [ ] 收件后 README 台账已更新
- [ ] 飞书文档拉取用 `--as user`（personal profile）
- [ ] 私有版含完整元信息（来源 URL/日期/参与人）
- [ ] 公开版已脱敏（个人信息/公司名/敏感数据）
- [ ] 不把密钥/appSecret 写进任何文件
